#!/usr/bin/env python3
"""Generate episodes 24-30 for FluentGe Podcast using edge-tts."""
...
EPISODES = [
    {
        "num": 24, "title": "At the Post Office", "file": "episode24-post-office.mp3",
...
async def generate_tts(text, voice, rate, outpath):
    ...
    print(f"  [tts] {outpath.name} ({voice}): {text[:50]}...")
...
async def generate_episode(ep):
    ...
    for j, clip_name in enumerate([
        ...
        *[f"{i+20:02d}_n{i+1}.mp3" for i in range(8)],
        ...
        elif clip_name in ["00_intro.mp3", "01_vocab_intro.mp3", "10_slow_intro.mp3", "19_normal_intro.mp3", "27_n8.mp3"]:
...
async def main():
    """Generate all episodes 24-30"""
    print("🎧 Generating episodes 24-30 with edge-tts...")
    
    for ep in EPISODES:
        await generate_episode(ep)
    
    print(f"\n🎉 Episodes 24-30 generated with edge-tts!")
